...
export default function UIPuzzle () {
  ...
  useEffect(() => {
    ...
    if (userId && usersData[userId] && usersData[userId].puzzleProgress) {
      setProgress(usersData[userId].puzzleProgress);
    }
  }, []);
  ...
  const moveTile = (index) => {
    ...
    if (newTiles.every((tile, i) => tile === i)) {
      ...
      const usersData = JSON.parse(localStorage.getItem("users")) || {};
      const userId = localStorage.getItem("currentUserId"); // Ahora es dinámico
    
      if (!userId) return; // Si no hay usuario logueado, salimos
    
      if (!usersData[userId]) {
        usersData[userId] = { data: {}, puzzleProgress: {} };
      }
    
      if (!usersData[userId].puzzleProgress[selectedLevel]) {
        usersData[userId].puzzleProgress[selectedLevel] = {};
      }
    
      if (
        !usersData[userId].puzzleProgress[selectedLevel][selectedSublevel] ||
        elapsedTime < usersData[userId].puzzleProgress[selectedLevel][selectedSublevel].bestTime
      ) {
        usersData[userId].puzzleProgress[selectedLevel][selectedSublevel] = {
          completed: true,
  ...
  onClick={() => {
    ...
    if (userId && usersData[userId]) {
      usersData[userId].puzzleProgress = {};
      localStorage.setItem("users", JSON.stringify(usersData));
      setProgress({});
    }
  }}
